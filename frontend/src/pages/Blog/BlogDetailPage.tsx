  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-screen-md px-4 py-6 sm:px-6">
        ...
        <Link to="/blog" className="mb-6 inline-block font-ui text-sm text-brand-500 hover:underline">
          ← Voltar ao blog
        </Link>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-white py-20 text-center dark:border-gray-700 dark:bg-gray-900">
            <h3 className="font-heading text-lg text-gray-900 dark:text-white">Publicação não encontrada</h3>
          </div>
        ) : (
          <article className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 sm:p-8">
            ...
            {post.category && (
              <span className="mb-3 inline-block rounded-full bg-brand-50 px-2.5 py-0.5 font-ui text-xs font-medium text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">
                {post.category.name}
              </span>
            )}
            <h1 className="mb-2 font-heading text-3xl text-gray-900 dark:text-white">{post.title}</h1>
            <p className="mb-6 font-ui text-sm text-gray-500 dark:text-gray-400">
              {post.author} · {formatDate(post.published_at)}
            </p>
            <div className="whitespace-pre-wrap font-body text-base leading-relaxed text-gray-700 dark:text-gray-300">
              {post.content}
            </div>
          </article>
        )}
